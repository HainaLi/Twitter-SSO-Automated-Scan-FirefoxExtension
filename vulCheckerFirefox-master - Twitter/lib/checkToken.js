...
var cleanup = function(){
	console.log("checkToken: cleanup"); 
...
var nextModule = function (success){
	console.log("checkToken: nextModule"); 
...
var levenshteinDistance = function(a, b){
	console.log("checkToken: levenshteinDistance"); 
...
var RequestRecord = function(){
	console.log("checkToken: RequestRecordule"); 
...
var ResponseRecord = function(){
	console.log("checkToken: ResponseRecord"); 
...
{
	ccc.log("checkToken: checkToken"); 

	if (!storageRecord.twitterDialogOAuthResponse) {ccc.log("Error: twitterOAuthResponse is undefined!"); return false;}
	var res = storageRecord.twitterDialogOAuthResponse.body;
	if (!ccc.usedFBSDK()) res = storageRecord.twitterDialogOAuthResponse.url;		
	ccc.log(res);
	if (typeof res == "undefined") {ccc.log("Error: facebookOAuthResponse URL/content empty!"); return false;}
	if (res.indexOf('oauth_token=')!=-1) {
		ccc.log("OAuth_token exists in this traffic.");
		ccc.log("Now try to verify this exploit");
		return true;
	}
	else {
		ccc.log("OAuth_token NOT spotted in this traffic.");
...
{
	console.log("checkToken: verifyThreats"); 
...
{
	console.log("checkToken: checkAgainstFilter"); 

	var i = 0;
	if (capturingPhase == inheritedPhase + 1 && (url.indexOf("https://api.twitter.com/oauth/")==0)) 
	{
	...
	else if (!ccc.usedFBSDK() && ccc.redirectDomain() != "" && capturingPhase == inheritedPhase + 2)
	{
		...
		if (redirectDomain[redirectDomain.length-1] == '/') redirectDomain = redirectDomain.substr(0,redirectDomain.length-1);
		if (url.startsWith(redirectDomain) && ccc.credentialsInserted() && ((url + additionalRedirectInfo).indexOf('oauth_token=')!=-1 || (url + additionalRedirectInfo).indexOf('code=')!=-1 || (url + additionalRedirectInfo).indexOf('signed_request=')!=-1)) {
			return true;
		}
	}
	return false;
...
function checkLoginButtonRemoved(exists){
	console.log("checkToken: checkLoginButtonRemoved"); 
...
function checkStructuralDiff(){
	console.log("checkToken: checkStructuralDiff"); 
...
{
	console.log("checkToken: processBuffer"); 
...
{
	console.log("checkToken: delayRefreshTestTab"); 
...
TracingListener.prototype =
{
    onDataAvailable: function(request, context, inputStream, offset, count)
    {
		...
		{
			if ((data.substr(0,42) == '<script type="text/javascript">var message' || data.substr(0,42) == '<script type="text/javascript">\nvar messag' || data.substr(0,56)=='for (;;);{"__ar":1,"payload":null,"jsmods":{"define":[["') && data.indexOf('oauth_token=')!=-1){
				ccc.log("Phase "+(inheritedPhase+2).toString()+": App using SDK: trying to modify access_token.\n");
				var head = data.substr(0, data.indexOf('oauth_token='));
				var tail = data.substr(data.indexOf('oauth_token='), data.length);
				...
				count = count + ("oauth_token="+ccc.accountsInfo()[0].access_token).length;
				tail = tail.substr(tail.indexOf('&'),tail.length);
				data = head + "oauth_token=" + ccc.accountsInfo()[0].access_token + tail;
...
var httpRequestObserver =
{
    observe: function(aSubject, aTopic, aData)
    {
        if (aTopic == "http-on-examine-response")
        {
			...
			if ((!ccc.usedFBSDK()) && uri.indexOf('oauth_token=')!=-1 && ccc.capturingPhase() == inheritedPhase+2)
			{
				...
				var tail = uri.substr(uri.indexOf('oauth_token='), uri.length);
				...
				var redirectURI = uri.substr(0,uri.indexOf('oauth_token='))+"oauth_token="+ccc.accountsInfo()[0].access_token+tail;
...
{
	console.log("checkToken: shouldClickLoginButton"); 
...
exports.shouldReturnLoginButtonInfoToVulChecker = function(){
	console.log("checkToken: shouldReturnLoginButtonInfoToVulChecker"); 
...
{
	console.log("checkToken: shouldAutomateSSO"); 
...
{
	console.log("checkToken: processLoaded"); 
